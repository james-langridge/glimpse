...
type DownloadSortKey = "time" | "code" | "email" | "location" | "device" | "browser";
...
function RecentDownloadCard({ download }: { download: RecentDownload }) {
  ...
  return (
    <div
      ...
      onClick={() => router.push(`/admin/links/${download.share_link_id}`)}
    >
...
export default function PhotoAnalytics({ photoId }: { photoId: string }) {
  const router = useRouter();
  const [data, setData] = useState<PhotoAnalyticsData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/photos/${photoId}/analytics?days=${days}`,
      );
      if (res.ok) {
                  ...
                  <tbody>
                    {sortedDownloads.map((dl) => (
                      <tr
                        ...
                        className="cursor-pointer border-b border-zinc-800/50 transition hover:bg-zinc-800/50"
                        onClick={() =>
                          router.push(`/admin/links/${dl.share_link_id}`)
                        }
                      >
                        <td className="py-2 text-zinc-400">
